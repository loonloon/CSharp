    ...
    internal class RadioStationController : IRadioStation
    {
        ...
        public async Task<List<RadioStation>> GetStations(string searchText)
        {
            using (var db = new Db())
            {
                ...
                if (!await db.RadioStations.AnyAsync() && await RefreshDirbleStations())
                {
        ...
        {
            stations = stations == null ? new List<RadioStation>() : stations.ToList();
            ...
            if (stations.Any())
            {
                using (var db = new Db())
                {
                    var urls = stations.Select(s => s.Url.ToLower()).ToArray();
                    var existing = await db.RadioStations.Where(s => urls.Contains(s.Url.ToLower())).ToListAsync();
                    stations = stations.Where(s => existing.All(es => !string.Equals(es.Url.ToLower(), s.Url.ToLower(), StringComparison.Ordinal))).ToList();
